...
#Outputs into secondary_actor.tsv
...
#Generating ranked list of secondary (actor[1]) actors and avg genre score: 
actors=[]
for n,ID in enumerate(list(actor[1].values())[0]):
    if ID:
        for k,v in ID.items():
            if v:
                if len(v)==1:
...
#Writing it to csv
df_rank_final.to_csv('../data/secondary_actor.tsv', sep= '\t', header=True)
