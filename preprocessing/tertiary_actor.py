...
#Outputs into tertiary_actor.csv
...
#Generating ranked list of tertiary (actor[2]) actors and avg genre score: 
actors=[]
for n,ID in enumerate(list(actor[2].values())[0]):
    if ID:
        for k,v in ID.items():
            if v:
                if len(v)==1:
...
#Writing it to csv
df_rank_final.to_csv('../data/tertiary_actor.tsv', sep= '\t', header=True)
